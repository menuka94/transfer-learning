...
import org.apache.spark.{SparkException, ml}

/**
 * A feature transformer that transform a vector to single fields.
 */
class VectorDisassembler (override val uid: String)
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val group =
      field.dataType match {
        case _: ml.linalg.VectorUDT => AttributeGroup.fromStructField(field)
